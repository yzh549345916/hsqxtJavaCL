...
public class 天擎EC {
    ...
    public static String EC高空下载(Date myDate,String fcstEle){
        try{

            JSONArray myFileLists=getECHeighNafpFileByElementAndTime(myDate,fcstEle,"100");
            ...
            if(myFileLists!=null&&myFileLists.size()>0){
                for (Object myList:myFileLists
                    ...
                    try{
                        ...
                        String myName = FileUtil.getParent(new ClassPathResource("config").getAbsolutePath(), 2) + "/区台数值预报文件/szyb/huanbao/ECMWF/" + format2 + "/high/ls/" +file_nameILE_NAME;
            ...
            if(myFileLists!=null&&myFileLists.size()>0){
                for (Object myList:myFileLists
                    ...
                    try{
                        ...
                        String file_nameILE_NAME=myfile.get("FILE_NAME").toString();
                        String file_url=myfile.get("FILE_URL").toString();
                        String myName = FileUtil.getParent(new ClassPathResource("config").getAbsolutePath(), 2) + "/区台数值预报文件/szyb/huanbao/ECMWF/" + format2 + "/surface/ls/" +file_nameILE_NAME;
                        if(!判断指定日期要素EC文件是否存在(filesize,myName)){
                            HttpUtil.downloadFile(file_url, FileUtil.file(myName), new StreamProgress(){
                                double jd=0;
                                @Override
    ...
    /**
     * 根据起报时间预报要素获取天擎下载EC高空列表
     * @param myDate 起报时间
     * @param fcstEle 预报要素
     * @return
     */
    public static JSONArray getECHeighNafpFileByElementAndTime(Date myDate,String fcstEle,String levelType){
        ...
        HashMap<String, String> params = new HashMap<String, String>();
        //必选参数
        params.put("dataCode", "NAFP_ECMF_C1D_ANEA_FOR");
        params.put("fcstEle", fcstEle);
        params.put("levelType", levelType);
        params.put("time", DateUtil.format(DateUtil.offsetHour(myDate, -8),"yyyyMMddHHmmss"));
        params.put("elements", "ELE_CODE,Datetime,FILE_SIZE,FILE_NAME,File_URL");
