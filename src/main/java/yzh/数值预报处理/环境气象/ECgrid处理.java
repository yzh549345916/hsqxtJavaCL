...
public class ECgrid处理 {
    ...
    public static List<高空要素Model> EC高空grid处理(Date sdate,String resourPath, List<站点信息> stations){
        List<高空要素Model> dataLists = new ArrayList<>();
        MeteoDataInfo aDataInfo = new MeteoDataInfo();
        aDataInfo.openNetCDFData(resourPath);
        List<LocalDateTime> tDimension = aDataInfo.getDataInfo().getTimes();
        Dimension zDim=aDataInfo.getDataInfo().getZDimension();

        LocalDateTime sLocalDate= LocalDateTimeUtil.of(DateUtil.offsetHour(sdate,-8));
        for(int l=0;l<zDim.getLength();l++){
            ...
            for (int i = 0; i < tDimension.size(); i++) {
                ...
                for(Variable variable:variables){
                    if(variable.isPlottable()){
                        ...
                        for (int k = 0; k < stations.size(); k++) {
                            站点信息 station = stations.get(k);
                            dataLists.add(new 高空要素Model((int)LocalDateTimeUtil.between(sLocalDate,tDimension.get(i)).toHours(), (int)Math.round(zDim.getDimValue(l)), myGridData.getValue(station.getLon(), station.getLat()), station.getID(), sdate));
                        }
                    }
                }
            }
